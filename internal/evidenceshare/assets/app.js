...
async function renderPublicRecord() {
  ...
  try {
    ...
    if (primary && publicPath(primary.href) && validSHA256(primary.sha256)) {
      frame.hidden = false; byId("screenshot").src = primary.href; byId("screenshot").alt = primary.alt_text || tr("evidence_asset", { id: primary.asset_id }); byId("image-link").href = primary.href;
      text(byId("source-label"), primary.source_ref || tr("source_not_disclosed")); text(byId("capture-label"), `${primary.width ? locale.number(primary.width) : "—"} × ${primary.height ? locale.number(primary.height) : "—"} · ${primary.media_type}`);
