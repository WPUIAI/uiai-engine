...
func sessionReadHandler(sm *vision.SessionManager) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if !ok {
			writeSessionContractError(w, http.StatusNotFound, "session_not_found", "session not found")
			return
		}
		...
		if req.Method == http.MethodGet {
			...
			if err != nil {
				writeSessionContractError(w, http.StatusBadRequest, "invalid_request", err.Error())
				return
			}
			options = parsed
		} else {
