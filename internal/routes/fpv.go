...
:root{color-scheme:dark;--bg:#050913;--bg2:#0b1220;--panel:rgba(11,18,32,.82);--panel-strong:rgba(15,23,42,.95);--card:rgba(2,6,23,.58);--text:#f8fafc;--sub:#b6c2d2;--muted:#7d8aa0;--line:rgba(148,163,184,.18);--line2:rgba(148,163,184,.30);--cyan:#67e8f9;--blue:#60a5fa;--violet:#a78bfa;--green:#34d399;--amber:#fbbf24;--red:#fb7185;--radius:5px;--gap:clamp(10px,1.1vw,18px);--pad:clamp(12px,1.1vw,18px);--fs-xs:clamp(10px,.68vw,12px);--fs-sm:clamp(12px,.82vw,14px);--fs-md:clamp(14px,.95vw,16px);--fs-lg:clamp(18px,1.45vw,24px);--fs-xl:clamp(26px,2.35vw,42px);--ease:cubic-bezier(.16,1,.3,1);--spring:cubic-bezier(.34,1.56,.64,1);--shadow:0 28px 90px rgba(0,0,0,.52)}*{box-sizing:border-box}html,body{height:100%;margin:0;overflow:hidden}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:radial-gradient(900px 620px at 14% -14%,rgba(103,232,249,.20),transparent 58%),radial-gradient(820px 560px at 95% 5%,rgba(167,139,250,.18),transparent 54%),linear-gradient(135deg,var(--bg),var(--bg2) 60%,#020617);color:var(--text);font-size:var(--fs-sm);letter-spacing:-.01em}body:before{content:"";position:fixed;inset:0;pointer-events:none;background-image:linear-gradient(rgba(255,255,255,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.026) 1px,transparent 1px);background-size:56px 56px;mask-image:linear-gradient(to bottom,rgba(0,0,0,.65),transparent 72%)}header{height:68px;padding:12px clamp(14px,1.7vw,24px);display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line);background:rgba(2,6,23,.68);backdrop-filter:blur(24px);position:relative;z-index:5}.brand{display:flex;align-items:center;gap:12px}.mark{width:38px;height:38px;border-radius:5px;display:grid;place-items:center;background:linear-gradient(135deg,var(--cyan),var(--violet));color:#020617;font-weight:950;box-shadow:0 14px 38px rgba(103,232,249,.22)}.title{font-size:var(--fs-lg);font-weight:900;letter-spacing:-.05em}.sub{color:var(--muted);font-size:var(--fs-xs);margin-top:2px}.statusbar{display:flex;gap:9px;align-items:center}.chip{display:inline-flex;align-items:center;gap:7px;padding:7px 10px;border:1px solid var(--line);border-radius:5px;background:rgba(15,23,42,.72);font-size:var(--fs-xs);font-weight:800;color:#dbeafe}.led{width:8px;height:8px;border-radius:5px;background:var(--green);box-shadow:0 0 0 5px rgba(52,211,153,.10),0 0 18px var(--green);animation:pulse 2.4s var(--ease) infinite}main{height:calc(100vh - 68px);display:grid;grid-template-columns:minmax(0,2fr) minmax(360px,1fr);gap:var(--gap);padding:var(--gap)}.stage{min-width:0;display:flex}.browser{width:100%;border:1px solid var(--line2);border-radius:var(--radius);overflow:hidden;background:linear-gradient(180deg,rgba(15,23,42,.92),rgba(2,6,23,.96));box-shadow:var(--shadow);display:flex;flex-direction:column;position:relative}.browser:before{content:"";position:absolute;inset:0;pointer-events:none;border-radius:inherit;background:linear-gradient(135deg,rgba(255,255,255,.13),transparent 20%,transparent 74%,rgba(103,232,249,.10));mix-blend-mode:screen}.browser:after{content:"";position:absolute;inset:52px 0 auto;height:24%;pointer-events:none;background:linear-gradient(180deg,transparent,rgba(103,232,249,.08),transparent);animation:scan 4.8s linear infinite;opacity:.38}.chrome{height:52px;display:grid;grid-template-columns:76px 1fr auto;gap:12px;align-items:center;padding:0 14px;background:rgba(15,23,42,.88);border-bottom:1px solid var(--line);backdrop-filter:blur(18px)}.lights{display:flex;gap:8px}.light{width:12px;height:12px;border-radius:5px;}.red{background:#ff5f57}.yellow{background:#febc2e}.green{background:#28c840}.addr{height:31px;display:flex;align-items:center;gap:8px;padding:0 13px;border:1px solid var(--line);border-radius:5px;background:rgba(2,6,23,.72);color:#dbeafe;font-size:var(--fs-xs);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.lock{color:var(--green)}.chrome-meta{font-size:10px;color:var(--muted);font-weight:900;text-transform:uppercase;letter-spacing:.08em}.viewport{flex:1;min-height:0;display:grid;place-items:center;background:#000}.viewport img{width:100%;height:100%;object-fit:contain;background:#000;transition:opacity .12s var(--ease)}.rail{height:100%;border:1px solid var(--line2);border-radius:var(--radius);overflow:hidden;background:var(--panel);backdrop-filter:blur(24px);box-shadow:var(--shadow);display:flex;flex-direction:column}.rail-head{padding:var(--pad);border-bottom:1px solid var(--line);background:rgba(15,23,42,.90)}.rail-title{display:flex;align-items:center;gap:9px;font-size:var(--fs-lg);font-weight:950;letter-spacing:-.055em}.rail-sub{color:var(--muted);font-size:var(--fs-xs);margin-top:3px}.signal{height:5px;margin-top:12px;border-radius:5px;background:linear-gradient(90deg,var(--green),var(--cyan),var(--violet),var(--green));background-size:220% 100%;animation:flow 3.2s linear infinite}.tabs{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;padding:10px;border-bottom:1px solid var(--line);background:rgba(2,6,23,.22)}.tab{border:1px solid transparent;border-radius:5px;background:transparent;color:var(--sub);padding:9px 7px;font-weight:850;font-size:var(--fs-xs);display:flex;gap:6px;align-items:center;justify-content:center;cursor:pointer}.tab.active{color:#fff;background:rgba(103,232,249,.10);border-color:rgba(103,232,249,.34);box-shadow:inset 0 1px rgba(255,255,255,.06)}.tab-panel{display:none;flex:1;overflow:auto;padding:var(--pad)}.tab-panel.active{display:block}.section-title{display:flex;align-items:center;justify-content:space-between;margin:0 0 10px;font-size:11px;text-transform:uppercase;letter-spacing:.12em;color:#dbeafe}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.card{position:relative;overflow:hidden;background:var(--card);border:1px solid var(--line);border-radius:5px;padding:11px;min-height:74px;transition:transform .28s var(--ease),border-color .2s var(--ease),background .28s var(--ease)}.card:hover{transform:translateY(-1px);border-color:rgba(103,232,249,.38)}.card:after{content:"";position:absolute;inset:-55% -70%;background:linear-gradient(115deg,transparent 42%,rgba(255,255,255,.16) 50%,transparent 58%);transform:translateX(-55%) skewX(-12deg);opacity:0}.card:hover:after,.card.sweep:after{animation:null-shimmer 1.05s var(--ease)}.label{display:flex;align-items:center;gap:6px;color:var(--muted);font-size:10px;font-weight:950;text-transform:uppercase;letter-spacing:.09em}.value{margin-top:7px;font-size:var(--fs-md);font-weight:850;line-height:1.15;word-break:break-word}.value.hero{font-size:var(--fs-xl);letter-spacing:-.06em}.hint{margin-top:5px;color:var(--muted);font-size:var(--fs-xs);line-height:1.25}.ok{color:var(--green)}.warn{color:var(--amber)}.bad{color:var(--red)}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}.span2{grid-column:1/-1}.tree{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:1.65;color:#cbd5e1;background:rgba(2,6,23,.45);border:1px solid var(--line);border-radius:5px;padding:12px}.timeline{display:grid;gap:9px}.event{display:grid;grid-template-columns:34px 1fr;gap:9px;padding:10px;border:1px solid var(--line);border-radius:5px;background:rgba(2,6,23,.46);animation:stream-in .42s var(--spring)}.glyph{width:28px;height:28px;border-radius:5px;display:grid;place-items:center;background:rgba(103,232,249,.10);border:1px solid rgba(103,232,249,.20)}.event-title{font-weight:850}.event-meta{color:var(--muted);font-size:var(--fs-xs);margin-top:2px}.controlbox{display:grid;gap:10px}textarea{width:100%;min-height:96px;resize:vertical;border:1px solid var(--line);border-radius:5px;background:rgba(2,6,23,.62);color:var(--text);padding:11px;outline:none}textarea:focus{border-color:rgba(103,232,249,.62);box-shadow:0 0 0 4px rgba(103,232,249,.10)}button.action{border:0;border-radius:5px;padding:12px 14px;background:linear-gradient(135deg,var(--cyan),var(--violet));color:#020617;font-weight:950;cursor:pointer}.empty{color:var(--muted);font-size:var(--fs-sm);padding:16px;border:1px dashed var(--line);border-radius:5px;background:rgba(2,6,23,.30)}.tag{letter-spacing:.12em;color:var(--muted);opacity:.72;transition:opacity .25s var(--ease);font:800 9px/1 ui-monospace,SFMono-Regular,Menlo,monospace;position:absolute;top:8px;right:9px}.card:hover .tag,.tag.always{opacity:1}.shine{opacity:0;pointer-events:none;z-index:2;background:linear-gradient(100deg,#0000,#ffffff0a 35%,#ffffff1a 50%,#ffffff0a 65%,#0000);width:110px;position:absolute;top:-30%;bottom:-30%;left:-140px;transform:skew(-14deg)}.card.sweep .shine,.card:hover .shine{animation:null-shimmer 1.05s var(--ease) forwards}.data-viz{margin-top:10px}.seismo-grid{height:74px;display:grid;grid-template-columns:repeat(48,1fr);align-items:end;gap:2px;padding:10px;background:rgba(2,6,23,.48);border:1px solid var(--line);border-radius:5px;overflow:hidden}.seismo-grid i{display:block;min-height:3px;background:linear-gradient(180deg,var(--cyan),rgba(96,165,250,.45));border-radius:2px;transform-origin:bottom;animation:seg-in .28s var(--ease) both}.glyph-grid,.health-grid{display:grid;grid-template-columns:repeat(12,1fr);gap:3px;padding:10px;background:rgba(2,6,23,.48);border:1px solid var(--line);border-radius:5px;}.glyph-grid i,.health-grid i{aspect-ratio:1;background:#1c1c1c;border-radius:2px;display:block;transform:scale(0);animation:cell-slam .38s var(--spring) forwards;transition:background .18s var(--ease),filter .22s var(--ease)}.glyph-grid i.l1,.health-grid i.l1{background:#153345}.glyph-grid i.l2,.health-grid i.l2{background:#1d5b73}.glyph-grid i.l3,.health-grid i.l3{background:#2a8ead}.glyph-grid i.l4,.health-grid i.l4{background:#67e8f9}.glyph-grid i.warn,.health-grid i.warn{background:var(--amber);animation:streak-wave 1.4s var(--ease) infinite}.glyph-grid i.bad,.health-grid i.bad{background:var(--red);animation:streak-wave .9s var(--ease) infinite}.ticker{display:flex;gap:6px;align-items:center;overflow:hidden;white-space:nowrap;color:var(--muted);font-size:var(--fs-xs);font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.ticker b{color:var(--text)}@keyframes pulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(.72);opacity:.58}}@keyframes scan{0%{transform:translateY(-35%)}100%{transform:translateY(380%)}}@keyframes flow{0%{background-position:0 50%}100%{background-position:220% 50%}}@keyframes shimmer{0%{opacity:0;transform:translateX(-55%) skewX(-12deg)}18%{opacity:1}100%{opacity:0;transform:translateX(55%) skewX(-12deg)}}@keyframes stream-in{0%{opacity:0;transform:translateY(10px) scale(.985)}100%{opacity:1;transform:none}}@media (prefers-reduced-motion:reduce){*,*:before,*:after{animation:none!important;transition:none!important}}@media (max-width:1023px){html,body{height:auto;overflow:auto}main{height:auto;grid-template-columns:1fr}.browser{height:clamp(420px,70vh,760px)}.rail{height:auto;min-height:620px}.tab-panel{max-height:none}}@media (max-width:767px){header{height:auto;padding:12px}.statusbar{display:none}main{padding:10px}.chrome{grid-template-columns:54px 1fr}.chrome-meta{display:none}.tabs{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}.browser{height:62vh}}@media (max-width:420px){.rail-title{font-size:18px}.tab{justify-content:flex-start}.browser{height:58vh}}
</style></head>
...
<aside class="rail"><div class="rail-head"><div class="rail-title"><span>✦</span><span>Realtime Control Center</span></div><div class="rail-sub">Categorized browser, diagnostics, repo, Focusa, and audit context.</div><div class="signal"></div><div class="ticker" id="ticker"><b>LIVE</b><span>waiting for telemetry frames</span></div></div><nav class="tabs"><button class="tab active" data-tab="overview">◉ Overview</button><button class="tab" data-tab="diagnostics">◇ Health</button><button class="tab" data-tab="repo">⌘ Repo</button><button class="tab" data-tab="focusa">✦ Focusa</button><button class="tab" data-tab="timeline">↯ Timeline</button><button class="tab" data-tab="control">✎ Control</button></nav>
...
<section id="repo" class="tab-panel"><h3 class="section-title">Repository context</h3><div class="grid"><div class="card"><div class="label">⌘ Project</div><div class="value">uiai-engine</div><div class="hint">Cloudflare FPV public viewer</div></div><div class="card"><div class="label">⑂ Branch</div><div class="value mono">main</div></div><div class="card"><div class="label">● Head</div><div class="value mono">live build</div><div class="hint">See latest git commit in operator log</div></div><div class="card"><div class="label">◧ Public host</div><div class="value mono">fpv.wpuiai.com</div></div><div class="span2 tree">uiai-engine/\n├─ internal/routes/\n│  └─ fpv.go          active viewer route\n├─ docs/\n│  └─ FPV_BROWSER_STYLE_GUIDE.md\n└─ cloudflare\n   └─ fpv.wpuiai.com /m/* → localhost:7456</div></div></section>
<section id="focusa" class="tab-panel"><h3 class="section-title">Focusa context</h3><div class="grid"><div class="card span2"><div class="label">✦ Current objective</div><div class="value">Professional realtime FPV browser cockpit</div><div class="hint">Summarized operator context; raw Focusa payloads stay out of the default UI.</div></div><div class="card"><div class="label">✓ Evidence</div><div class="value mono">git + live FPV proof</div></div><div class="card"><div class="label">⛨ Drift guard</div><div class="value">Keep public host path-gated to /m/*</div></div></div></section>
<section id="timeline" class="tab-panel"><h3 class="section-title">History timeline</h3><div id="stream" class="timeline"></div></section>
<section id="control" class="tab-panel"><h3 class="section-title">Operator control</h3><div class="controlbox"><textarea id="msg" placeholder="Send a note to the audited action stream"></textarea><button class="action" onclick="sendMsg()">Send audited note</button><div id="controlResult" class="hint"></div><div class="empty">Click, fill, and keypress controls are accepted by the API for control-enabled shares. This panel currently exposes audited notes first to keep the UI safe and focused.</div></div></section>
</aside></main>
<script>
...
function addStream(kind,msg,glyph='↯'){stream.unshift({t:new Date(),kind,msg,glyph}); stream=stream.slice(0,24); const el=$('stream'); if(el) el.innerHTML=stream.length?stream.map(e=>'<div class="event"><div class="glyph">'+e.glyph+'</div><div><div class="event-title">'+e.kind+'</div><div class="event-meta">'+e.t.toLocaleTimeString()+' · '+e.msg+'</div></div></div>').join(''):'<div class="empty">No events yet.</div>'}
...
addStream('Viewer connected','FPV cockpit online','◉'); statusTick(); frameTick(); setInterval(statusTick,1500); setInterval(frameTick,250);
</script></body></html>`))
