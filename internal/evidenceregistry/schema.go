...
var schemaV1 = []string{
	...
	`CREATE TABLE IF NOT EXISTS project_projection (
		...
		scope_safety TEXT NOT NULL,
		source_schema TEXT NOT NULL,
		source_revision TEXT NOT NULL DEFAULT '',
		observed_at INTEGER NOT NULL
	) STRICT`,
	`CREATE UNIQUE INDEX IF NOT EXISTS project_identity ON project_projection(project_id, fingerprint)`,
	`CREATE TABLE IF NOT EXISTS provider_graph_state (
		project_ref TEXT PRIMARY KEY,
		graph_digest TEXT NOT NULL,
		item_count INTEGER NOT NULL,
		updated_at INTEGER NOT NULL,
		FOREIGN KEY(project_ref) REFERENCES project_projection(project_ref) ON DELETE CASCADE
	) STRICT`,
	`CREATE TABLE IF NOT EXISTS provider_work_item (
		project_ref TEXT NOT NULL,
		...
		acceptance_refs_json TEXT NOT NULL DEFAULT '[]',
		spec_refs_json TEXT NOT NULL DEFAULT '[]',
		external_ref TEXT NOT NULL DEFAULT '',
		source_authority TEXT NOT NULL,
		binding_state TEXT NOT NULL,
		observed_at INTEGER NOT NULL,
		PRIMARY KEY(project_ref, work_item_ref),
		FOREIGN KEY(project_ref) REFERENCES project_projection(project_ref) ON DELETE CASCADE
	) STRICT`,
	`CREATE INDEX IF NOT EXISTS provider_work_item_list ON provider_work_item(project_ref, status, priority, item_type, item_id)`,
	`CREATE INDEX IF NOT EXISTS provider_work_item_digest ON provider_work_item(project_ref, digest)`,
	`CREATE VIRTUAL TABLE IF NOT EXISTS provider_work_item_search USING fts5(
		project_ref UNINDEXED,
		work_item_ref UNINDEXED,
		item_id,
		title,
		description,
		external_ref,
	...
	`CREATE TABLE IF NOT EXISTS work_item_edge (
		project_ref TEXT NOT NULL,
		...
		source_authority TEXT NOT NULL,
		observed_at INTEGER NOT NULL,
		PRIMARY KEY(project_ref, source_ref, target_ref, relation),
		FOREIGN KEY(project_ref) REFERENCES project_projection(project_ref) ON DELETE CASCADE
	) STRICT`,
	`CREATE INDEX IF NOT EXISTS work_item_edge_reverse ON work_item_edge(project_ref, target_ref, relation, source_ref)`,
