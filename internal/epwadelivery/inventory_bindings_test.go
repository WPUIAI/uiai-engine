...
func TestConsumerAdaptersRequireReadyEPWA(t *testing.T) {
	checks := []struct {
		...
		{consumer: "native Pi", file: "../../.pi/extensions/uiai-engine.ts", tokens: []string{"assertEvidenceDelivery(data, requiresEvidence)", "evidenceScopeHeaders(evidenceRequestScope.getStore())", "./uiai/epwa-contract.mjs"}},
		{consumer: "shared Pi/MCP delivery validator", file: "../../.pi/extensions/uiai/epwa-contract.mjs", tokens: []string{"findRawArtifactField(value)", "findNonReadyArtifactDelivery(value)", "url.protocol !== \"https:\"", "required evidence envelope missing"}},
		{consumer: "MCP compatibility export", file: "../../mcp/epwa-contract.mjs", tokens: []string{"export * from \"../.pi/extensions/uiai/epwa-contract.mjs\""}},
