...
- [Focusa](https://github.com/Startempire-Wire/focusa) handoff: `browser_open` accepts `focusa_scope`; diagnostics and evidence flows preserve project/workpoint scope; `/api/tools/graph` exposes [Focusa](https://github.com/Startempire-Wire/focusa)-aware related-tool routes. Stable evidence handles are documented for diagnostics, errors, search results, browser reads/snapshots, screenshots, and shares. The Pi extension can compose `uiai.focusa_research_diagnostics_packet.v1` with `uiai_focusa_packet_build`; HTTP/MCP/CLI packet endpoint parity is intentionally deferred while `/api/tools/graph` advertises the workflow and existing response metadata. The iterative hand-in-glove roadmap is in [`docs/UIAI_FOCUSA_PI_HAND_IN_GLOVE_SPEC.md`](docs/UIAI_FOCUSA_PI_HAND_IN_GLOVE_SPEC.md).
