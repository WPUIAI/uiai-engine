...
- MCP bridge: [`mcp/browser-session-mcp.mjs`](mcp/browser-session-mcp.mjs) exposes browser/session tools plus `uiai_agent_card`, `uiai_tool_search`, `uiai_tool_graph`, `uiai_health`, `uiai_status`, `browser_search`, and `browser_read`; `tools/list` normalizes these core tools even when a running engine returns stale metadata.
