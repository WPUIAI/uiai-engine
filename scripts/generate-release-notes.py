...
def bound_release_notes(md, compare_url, max_chars=DEFAULT_MAX_CHARS):
    """Keep release notes within the GitHub API limit without silent truncation."""
    if max_chars <= 0 or len(md) <= max_chars:
        return md
    trailer = (
        "\n\n---\n\n"
        "### Release-note detail truncated\n\n"
        "The deterministic detail exceeded the publication budget. "
        f"Review the [full repository comparison]({compare_url}) for the complete audit trail.\n"
    )
    if len(trailer) >= max_chars:
        raise ValueError("release-note character budget is too small for truncation disclosure")
    prefix = md[: max_chars - len(trailer)]
    if "\n" in prefix:
        prefix = prefix.rsplit("\n", 1)[0]
    return prefix.rstrip() + trailer
...
def generate(tag, range_arg, output, preview=False, max_chars=DEFAULT_MAX_CHARS):
...
"""
    ...
    md = bound_release_notes(md, compare_url, max_chars)
    if preview:
        print(md)
        return
...
def main():
    ...
    args = p.parse_args()
    generate(args.tag, args.range_arg, args.output, args.preview, args.max_chars)
