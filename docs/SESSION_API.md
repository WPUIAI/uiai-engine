...
Packet parity status: Iteration 1 composes `uiai.focusa_research_diagnostics_packet.v1` in the Pi extension (`uiai_focusa_packet_build`) from existing UIAI `search`, `browser_read`, `browser_diagnostics`, and structured error responses. `/api/tools/graph` advertises the `focusa_research_packet` workflow and schema for MCP/CLI route planning, but UIAI does **not** yet expose `POST /api/agent/research-packet`; MCP/CLI callers should compose from existing response `focusa` metadata or use the Pi tool until the endpoint lands.
