...
| `/api/markdown` Source-to-Markdown | loopback-public remote-auth | `uiai_source_to_markdown`; `uiai_browser_read format=markdown` | `source_to_markdown`; `browser_read format=markdown` | `scripts/uiai markdown <url>`; `scripts/uiai session read --format markdown` | `uiai-source-markdown:sha256:<prefix>` plus `uiai.source_markdown_record.v1` records | `scripts/smoke-agent-integrations.sh`, `scripts/check-tool-parity.sh`, `go test ./internal/routes` | Full agent parity for MVP adapters | Generic browser Markdown plus `github_public` and `reddit_public`; X fallback remains roadmap. |
