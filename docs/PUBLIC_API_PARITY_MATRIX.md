...
| `/api/tools/*` | public discovery | `pi_uiai_agent_card`, `pi_uiai_tool_search`, `pi_uiai_tool_graph` | `uiai_agent_card`, `uiai_tool_search`, `uiai_tool_graph` | `scripts/uiai tools ...` | tool graph workflow refs and docs/examples metadata | `scripts/smoke-agent-integrations.sh` | Full agent parity | Generated tool defs remain authoritative; `/api/tools/docs` gives lightweight docs/examples metadata without loading full markdown. |
| `/api/search`, `/api/search/providers` | loopback-public remote-auth | `uiai_search` | `browser_search` | HTTP examples; `scripts/uiai-open-result.sh`; packet smoke uses search | `uiai-search:<provider>:<query-hash>:<rank>` | `scripts/smoke-agent-integrations.sh`, `scripts/smoke-focusa-packet.sh` | Full agent parity | Brave default plus keyless Wikipedia second provider; provider metadata redacts keys and reports degraded status/cache TTL. |
...
| `/api/markdown` Source-to-Markdown | loopback-public remote-auth | `uiai_source_to_markdown`; `uiai_browser_read format=markdown` | `source_to_markdown`; `browser_read format=markdown` | `scripts/uiai markdown <url>`; `scripts/uiai markdown <url> --format jsonl`; `scripts/uiai session read --format markdown` | `uiai-source-markdown:sha256:<prefix>` plus `uiai.source_markdown_record.v1` records, `uiai.source_markdown_chunk.v1` chunk refs, JSONL output, and `wpuiai.research_card` | `scripts/smoke-agent-integrations.sh`, `scripts/check-tool-parity.sh`, `go test ./internal/routes` | Full agent parity for MVP adapters | Generic browser Markdown plus `github_public`, `reddit_public`, `x_public`, `hackernews_public`, `youtube_public`; explicit JSONL/chunk output is available for adapters with records; WPUIAI plugin UI/save is implemented via `wpuiai_research` private posts. |
