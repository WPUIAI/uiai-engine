<script lang="ts">
  ...
  import { discoverFocusaDaemons, focusaDaemonSummary, type FocusaDaemonConnection } from "$lib/focusa-daemon-discovery";
  ...
  onMount(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { commandOpen = false; workspaceMenuOpen = false; contextOpen = false; activityOpen = false; }
...
<div class="app-shell">
  <aside class:compact={preferences.mode === "compact" || constrainedViewport} class="sidebar" aria-label="Primary navigation" style={`--sidebar-width: ${preferences.width_px}px`}>
    ...
    <div class="sidebar-bottom"><div class="connection-block" title={`UIAI Engine Cockpit ${APP_VERSION}`}><div class="connection"><span class:healthy={engineStatus === "healthy"} class="status-dot"></span><span>Engine {engineStatus}</span></div><div class="connection"><span class:healthy={focusaConnections.some((item) => item.status === "connected")} class="status-dot"></span><span>{focusaDaemonSummary(focusaConnections)}</span></div><div class="version-status"><strong>v{APP_VERSION}</strong><span>{APP_CHANNEL} channel</span></div></div>{#each footerDestinations as item}<a class="settings-button" class:active={activeWorkspaceId === item.id} href={item.route} aria-current={activeWorkspaceId === item.id ? "page" : undefined} onclick={() => selectWorkspace(item.id)}><span aria-hidden="true">{item.icon}</span><span>{item.label}</span></a>{/each}</div>
  </aside>
...
<style>
  ...
  .sidebar { position: sticky; top: 0; z-index: 10; width: var(--sidebar-width); min-width: var(--sidebar-width-min); max-width: var(--sidebar-width-max); height: 100vh; box-sizing: border-box; flex: 0 0 var(--sidebar-width); display: flex; flex-direction: column; overflow: hidden; padding: 18px 12px 12px; background: var(--sidebar-surface); border-right: 1px solid var(--sidebar-divider); transition: width var(--sidebar-transition), flex-basis var(--sidebar-transition); }
  ...
  .sidebar-bottom { margin-top: auto; display: grid; gap: 5px; padding: 14px 4px 0; border-top: 1px solid var(--color-border); } .connection-block { display: grid; gap: 5px; margin-bottom: 3px; padding: 8px; border: 1px solid var(--color-border-strong); border-radius: 10px; background: var(--color-surface-elevated); } .connection { display: flex; align-items: center; gap: 8px; color: var(--color-text-muted); font-size: 11px; } .version-status { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; color: var(--color-text-muted); font-size: 10px; } .version-status strong { color: var(--color-text); font-size: 12px; letter-spacing: -.01em; } .status-dot, .update-dot { width: 7px; height: 7px; display: inline-block; border-radius: 50%; background: var(--color-success); box-shadow: 0 0 0 3px color-mix(in srgb, var(--color-success) 14%, transparent); } .connection .status-dot { background: var(--color-warn); box-shadow: 0 0 0 3px color-mix(in srgb, var(--color-warn) 14%, transparent); } .connection .status-dot.healthy { background: var(--color-success); box-shadow: 0 0 0 3px color-mix(in srgb, var(--color-success) 14%, transparent); } .settings-button { padding: 7px 8px; }
  .sidebar.compact .brand { padding-inline: 7px; } .sidebar.compact .brand > div:last-child, .sidebar.compact .sidebar-heading > span, .sidebar.compact .group-heading span:first-child, .sidebar.compact .nav-label, .sidebar.compact .nav-state, .sidebar.compact .connection span:last-child, .sidebar.compact .version-status span, .sidebar.compact .settings-button span:last-child { display: none; } .sidebar.compact .connection-block { padding: 7px 5px; } .sidebar.compact .connection, .sidebar.compact .version-status { justify-content: center; } .sidebar.compact .version-status strong { font-size: 9px; } .sidebar.compact .sidebar-heading, .sidebar.compact .group-heading { justify-content: center; padding-inline: 0; } .sidebar.compact .workspace-menu-button { font-size: 12px; } .sidebar.compact .nav-item, .sidebar.compact .settings-button { justify-content: center; padding-inline: 7px; } .sidebar.compact .active-dot { position: absolute; right: 4px; }
  ...
  @media (prefers-reduced-motion: reduce) { .sidebar { transition: none; } } @media (max-width: 780px) { .sidebar { width: var(--sidebar-width-compact); min-width: var(--sidebar-width-compact); flex-basis: var(--sidebar-width-compact); padding-inline: 8px; } .topbar { padding: 0 16px; } .scope-pill { display: none; } .content { padding: 26px 18px 44px; } }
