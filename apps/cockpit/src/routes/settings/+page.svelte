<script lang="ts">
  ...
  async function testConnection() {
    ...
    testing = true;
    connectionResult = "";
    try { const result = await engineClient.health(); connectionResult = `${result.status} · ${result.service || "UIAI Engine"}`; } catch (cause) { connectionResult = cause instanceof Error ? cause.message : "Connection failed"; } finally { testing = false; }
  }
