<script lang="ts">
  import { humanBytes, packageThumbnailURL, secureEvidenceURL, sourceHost, type EvidenceShareManifest, type EvidenceSharePacket, type EvidenceShareVerification } from "$lib/evidence-share";
  ...
  let expanded = false, thumbnailFailed = false;
  $: thumbnailURL = packageThumbnailURL(packet);
  $: recordURL = secureEvidenceURL(packet.artifact_url);
  $: portableURL = secureEvidenceURL(packet.portable_url);
...
<article class="share-card" data-status={manifest?.availability || "ready"}>
  <div class="preview">{#if thumbnailURL && !thumbnailFailed}<img src={thumbnailURL} alt="" loading="lazy" decoding="async" on:error={() => thumbnailFailed = true} />{:else}<strong>{manifest?.format || "Evidence package"}</strong><p>Open the read-only record to view its contents.</p>{/if}</div>
  <div class="body">
    ...
    <div class="actions">{#if recordURL}<a class="primary" href={recordURL} target="_blank" rel="noopener noreferrer">View evidence <span>↗</span></a>{:else}<span>No secure record link available</span>{/if}{#if portableURL}<a href={portableURL} target="_blank" rel="noopener noreferrer">Portable package ↗</a>{/if}<button type="button" aria-expanded={expanded} on:click={toggle}>{expanded ? "Hide details" : "Inspect"}</button></div>
