<script lang="ts">
  import { humanBytes, secureEvidenceURL, sourceHost, type EvidenceShareManifest, type EvidenceSharePacket, type EvidenceShareVerification } from "$lib/evidence-share";
...
<article class="share-card" data-status={manifest?.availability || "ready"}>
  <div class="preview"><strong>{manifest?.format || "Evidence package"}</strong><p>Open the read-only record to view its contents.</p></div>
  <div class="body">
