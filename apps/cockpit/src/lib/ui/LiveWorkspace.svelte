<script lang="ts">
  ...
  async function refreshSession(session: BrowserSession) {
    selected = session;
    error = "";
    try { [screenshot, diagnostics] = await Promise.all([engineClient.screenshot(session.id), engineClient.diagnostics(session.id)]); } catch (cause) { error = cause instanceof Error ? cause.message : "The session inspection request failed."; }
  }
  ...
  async function closeSession(session: BrowserSession) {
    try { requireScopedMutation(savedScope(), "Close browser session"); await engineClient.closeSession(session.id); sessions = sessions.filter((item) => item.id !== session.id); if (selected?.id === session.id) { selected = null; screenshot = null; diagnostics = null; } } catch (cause) { error = cause instanceof Error ? cause.message : "The session could not be closed."; }
  }
...
<div class="screen">
  ...
  {#if selected}<aside class="screen-card pad inspector" aria-live="polite"><div class="screen-toolbar"><div><p class="screen-kicker">Inspect session</p><h2>{selected.title || selected.url}</h2><p>{selected.url}</p></div><button class="screen-button" type="button" onclick={() => { selected = null; diagnostics = null; screenshot = null; }}>Close inspector</button></div><div class="inspector-grid"><span><small>Session</small><strong>{selected.id}</strong></span><span><small>Engine</small><strong>{engineUrl()}</strong></span><span><small>State</small><strong>Live</strong></span></div><div class="inspector-detail"><p class="screen-kicker">Details</p><span>{diagnostics ? `${String(diagnostics.url || selected.url)} · ${String(diagnostics.title || "Untitled")}` : "Capture or inspect to load session diagnostics."}</span></div><div class="session-controls"><input bind:value={navigationUrl} aria-label="Navigate session to URL" placeholder="Navigate to a URL" onkeydown={(event) => event.key === "Enter" && navigateSession()} /><button class="screen-button primary" type="button" onclick={navigateSession} disabled={!navigationUrl.trim()}>Navigate</button><button class="screen-button" type="button" onclick={() => refreshSession(selected!)}>Capture screenshot</button></div>{#if screenshot?.screenshot}<img class="session-screenshot" src={`data:image/${screenshot.format || "jpeg"};base64,${screenshot.screenshot}`} alt={`Current browser view of ${screenshot.url || selected.url}`} />{:else}<div class="empty-inline">Capture a screenshot to inspect the current browser surface.</div>{/if}</aside>{/if}
</div>
